...
class MyGUI(QMainWindow):
    def __init__(self):
        ...
        self.pushButton.setStyleSheet("background-color:orange");("Arial",15)
    ...
    def lap_reset(self):
        if self.running:
            self.label_2.setText(self.label_2.text()+f"(Lap{self.lap}-Passed:{self.format_time_string(self.passed) }"f"-Difference:{self.format_time_string(self.passed - self.previous_passed)})")
            self.lap += 1
    ...
    def format_time_string(self, time_passed):
        ...
        hours = mins // 60
        day = hours // 24
        return f"{int(day):02d}:{int(hours):02d}:{int(mins):02d}:{int(secs):02d}"
